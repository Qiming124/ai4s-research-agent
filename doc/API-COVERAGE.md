...
| Memory | GET/POST | `/v1/memory/structured/{id}/versions` | — | ✓ | H | L4 版本 |
| Memory | POST | `/v1/memory/structured/{id}/edges` | — | ✓ | H | 图谱边 |
| Theory | GET | `/v1/theory/workspace` | ✓ | ✓ | A | 工作区列表 |
| Theory | GET/PUT | `/v1/theory/workspace/{path}` | ✓ | ✓ | H | 读写文件 |
...
| Sync | POST | `/v1/sync/metadata` | — | ✓ | M | 云端同步 |
| Sync | GET | `/v1/sync/audit/{id}` | — | ✓ | M | 审计日志 |
| Jupyter | GET | `/v1/jupyter/template` | ✓ | ✓ | H | 模板 |
...
课题选择 → 新建会话(自动关联) → 文献导入(arXiv/PDF) → 对话(SSE流水线)
  → 自动切Tab → 验证仪表盘刷新 → 导出 LaTeX/DOCX/PDF(按课题)
```
